...
simulable} (approximable by a classical kernel), and whether it yields a real
\emph{generalization advantage} over strong classical baselines. We study what spectral
criteria of the quantum kernel matrix---effective rank, spectral entropy, concentration
index, gap ratio---can and cannot predict, using a self-contained state-vector simulator
(no external quantum libraries), four families of feature maps, six tasks with controlled
structure and difficulty, and tuned classical baselines (RBF with validation-based kernel
width, random Fourier features, linear and polynomial kernels). Across 721 configurations
we find that spectral criteria reliably predict classical \emph{simulability} (top-5
energy: $\rho=-0.80$, $p=1.2\times10^{-15}$; spectral entropy: $\rho=+0.67$,
$p=1.0\times10^{-9}$, Benjamini--Hochberg corrected), but they do not predict quantum
\emph{advantage}: the global correlation is indistinguishable from zero
($|\rho|\lesssim0.07$, $p\ge0.31$), and even its sign is unstable across tasks
($\rho=+0.38$ in a single-task setting, $\rho=-0.63$ within the cross-term task,
null elsewhere). The dominant predictor of advantage is the degree to which classical
baselines saturate a task: advantage correlates strongly and negatively with the best
classical test accuracy ($\rho=-0.65$, $p=2\times10^{-27}$), and the fraction of
configurations with positive advantage tracks classical saturation monotonically
(38.9\% at a baseline of 0.55, 0\% at 0.86 and above). Controlled sweeps identify the
feature map's frequency scale as the decisive design knob for spectral spread and
simulability ($\rho=+1.000$, $p=1.4\times10^{-24}$), and show that spectral criteria are
strongly data-dependent. These results hold at $8$--$16$ qubits, on four real datasets
(where positive advantage is never observed, $0/48$ configurations), and under noise:
depolarization preserves the central relations, while amplitude damping reveals a
boundary---spectral collapse stops indicating classical approximability
($\rho=+0.04$ versus $+0.35$). We further test two candidate ``task--map matching'' criteria
(spectral spread ratio and kernel-target alignment): the global predictive power of
kernel-target alignment ($\rho=-0.42$, $p=1.7\times10^{-10}$) is entirely absorbed by
classical task difficulty (residual $\rho=+0.001$, $p=0.99$). The results draw a sharp
boundary for spectral pre-screening and provide concrete evaluation guidelines for
...
spread and simulability ($\rho=+1.000$, $p=1.4\times10^{-24}$), while entanglement
...
\item \textbf{exp10 (16-qubit scaling)}: 24 configurations at $16$ qubits completes the
scaling scan.
\item \textbf{exp11 (amplitude damping)}: 36 configurations under single-qubit
amplitude-damping noise ($\gamma\in\{0,0.05,0.2,0.4\}$).
\end{itemize}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig1_difficulty.pdf}
...
scale is decisive: effective rank and RFF error both follow $s$ with $\rho=+1.000$
($p=1.4\times10^{-24}$). A larger frequency scale spreads the Fourier content of the
...
has essentially no effect on either quantity ($\rho=+0.20$, ns), a counterintuitive
result in light of the folklore that entanglement drives concentration. For the IQP
family, depth moves the RFF error ($\rho=+0.90$, $p=0.037$) but not the effective rank
...
\midrule
frequency scale (high-dim) & $+1.000$ & $+1.000$ & $p=1.4\times10^{-24}$ \\
entanglement strength (variational) & $+0.200$ & $-0.738$ & ns \\
depth (IQP) & $+0.400$ & $+0.900$ & rank ns; RFF $p=0.037$ \\
\bottomrule
\end{tabular}
\end{table}
...
approximation error. Both follow $s$ perfectly monotonically ($\rho=+1.000$).}
\label{fig:freqscale}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{fig4_scaling.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{fig5_alignment.pdf}
...
channels; correlated noise and measurement noise are not included. Real-data
experiments use PCA preprocessing to eight features and a single classifier (kernel
SVM) with fixed shrinkage. The task family, while deliberately diverse, still includes
only one quantum-agnostic real dataset family, and the matching-criterion analysis is
confined to the six tasks of exp03. The RFF family approximates translation-invariant
kernels; simulability statements should be read relative to that reference class.
Finally, all simulations are noiseless except exp08, and classical surrogates beyond
RFF and Nystr\"om (e.g., tensor-network contractions) are not considered.
...
no image-generation model was used for any figure or table visual. The author
reviewed and approved all code, results, and figures.
...
and are included as arXiv ancillary files with the manuscript. All experiments are
reproducible with NumPy, SciPy, scikit-learn, and matplotlib only.
