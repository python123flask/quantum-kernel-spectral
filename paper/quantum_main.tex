...
width, random Fourier features, linear and polynomial kernels). Across 781 configurations we find that
...
\item \textbf{exp06 (scaling)}: 48 configurations at $8$ and $12$ qubits ($N=250$,
two tasks) plus 3 pilot configurations at $16$ qubits ($N=150$); checks whether
the central relations survive larger systems.
...
alignment $\mathrm{TA}(K,yy^{\top})$ (reanalysis of exp03; no new configurations).
\item \textbf{exp10 (16-qubit scan)}: 24 configurations at $16$ qubits ($N=250$,
two tasks), completing the scaling scan of exp06.
\item \textbf{exp11 (amplitude damping)}: 36 configurations under single-qubit
amplitude-damping noise ($\gamma\in\{0,0.05,0.2,0.4\}$, density-matrix simulation).
\end{itemize}
...
law. We conclude that spectral criteria do not predict generalization advantage in a
task-independent way, and that apparent ``spectral pre-screening for advantage'' results
from pooling within a favourable task--map combination.
...
Two observations guard against reading this association as a mere consequence of
Eq.~\eqref{eq:adv}. At fixed quantum accuracy, a stronger classical baseline lowers the
advantage by definition; the content of the result lies in how the quantum accuracy
itself behaves. Across tasks, the mean quantum accuracy varies from 0.49 to 0.77 while
the best classical baseline varies from 0.52 to 0.94, and the quantum shortfall grows
with the classical baseline: the gap is 0.016 on periodic-coupled and $-0.004$ on
noisy-harmonic (where classical baselines remain near chance) but 0.225 on cross-term
and 0.173 on breast cancer (where they exceed 0.86). The quantum kernel does not
compensate for a strong classical competitor; it falls furthest behind exactly there.
Second, the association also holds within a fixed task, where both accuracies vary
across feature-map variants and seeds, so it is not merely a between-task effect.
...
($\rho$ of effective rank vs.\ RFF error) strengthens with system size, from 5 to
16 qubits. Right: on four real datasets the classical baseline is near-saturated and
quantum advantage is never positive.}
\label{fig:scaling}
\end{figure}
...
correlates with advantage globally ($\rho=-0.42$). Right: after rank-space residualization on the
